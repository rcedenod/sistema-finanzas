...
export class Budgets {
    ...
    async loadBudgets() {
        try {
            this._budgets = await this._db.getBudgets();
    ...
    render() {
        ...
        formArea.classList.add('budget-form-area');
        ...
        budgetsView.appendChild(formArea);
    ...
    updateCategorySelects() {
        ...
        if (categoryOptions.length > 0) {
            this._budgetCategorySelect.setValue(categoryOptions[0].value);
        }
    }
    ...
    async renderBudgetsList() {
        ...
        this._budgets.forEach(budget => {
            ...
            const categoryName = this._categories.find(cat => cat.id === budget.categoryId)?.name || 'Desconocida';
    ...
    clearForm() {
        ...
        if (this._categories.length > 0) {
            this._budgetCategorySelect.setValue(this._categories[0].id);
        } else {
